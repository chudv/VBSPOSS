...
namespace VBSPOSS.Controllers
{
    ...
    {
        public PermissionController(ILogger<BaseController> logger, IAdministrationService administrationService, ISessionHelper sessionHelper) : base(logger, administrationService, sessionHelper)
        {
        }
        ...
        {
            var list = _administrationService.GetPermissions();
        ...
        public ActionResult Create([DataSourceRequest] DataSourceRequest request, Data.Models.Permission model)
        {
            try
            {
                if (model != null && ModelState.IsValid)
                {
                    if (model.Id == 0)
                    {
                        model.Id = _administrationService.CreatePermission(model);
                    }
                    else
                    {
                        model.Id = _administrationService.UpdatePermission(model);
                    }
                }
        ...
        public ActionResult Edit([DataSourceRequest] DataSourceRequest request, Data.Models.Permission model)
        {
            try
            {
                if (model != null && ModelState.IsValid)
                {
                    int result = _administrationService.UpdatePermission(model);
                    if (result == 0)
                    {
        ...
        public ActionResult Delete([DataSourceRequest] DataSourceRequest request, Data.Models.Permission model)
        {
            try
            {
                if (model != null && ModelState.IsValid)
                {
                    int result = _administrationService.DeletePermission(model);
                    if (result == 0)
                    {
